...
impl Paths {
    /// `$UMOJA_HOME`, else `$PRIME_AGENT_HOME`, else `~/.umoja`, else `~/.prime/agent`.
    pub fn resolve() -> Result<Self> {
        if let Some(explicit) = std::env::var_os("UMOJA_HOME").or_else(|| std::env::var_os("PRIME_AGENT_HOME")) {
            return Self::at(PathBuf::from(explicit));
        }

        if let Some(home) = home_dir() {
            let umoja_dir = home.join(".umoja");
            if umoja_dir.exists() || !home.join(".prime").join("agent").exists() {
                if Self::at(umoja_dir.clone()).is_ok() {
                    return Self::at(umoja_dir);
                }
            }
            let prime_dir = home.join(".prime").join("agent");
            if Self::at(prime_dir.clone()).is_ok() {
                return Self::at(prime_dir);
            }
        }
        ...
        Self::at(xdg.join("umoja"))
    }

    pub fn at(root: PathBuf) -> Result<Self> {
        let paths = Self { root };
        std::fs::create_dir_all(paths.root())
            .map_err(|error| DomainError::adapter("create umoja home", error))?;
        Ok(paths)
    }
