...
impl KernelLanguage {
    pub fn parse(text: &str) -> Result<Self> {
        match text.trim().to_ascii_lowercase().as_str() {
            "rhai" | "rust" | "rs" | "umoja" => Ok(Self::Rhai),
            ...
            other => Err(DomainError::invalid(format!(
                "unknown kernel language '{other}'; expected rhai, rust, python, node or shell"
            ))),
        }
    }
...
mod tests {
    ...
    #[test]
    fn parses_rhai_and_rust_aliases() {
        assert_eq!(KernelLanguage::parse("rhai").unwrap(), KernelLanguage::Rhai);
        assert_eq!(KernelLanguage::parse("rust").unwrap(), KernelLanguage::Rhai);
        assert_eq!(KernelLanguage::parse("rs").unwrap(), KernelLanguage::Rhai);
        assert_eq!(KernelLanguage::parse("umoja").unwrap(), KernelLanguage::Rhai);
        assert!(KernelLanguage::Rhai.holds_objects());
    }
