...
use umoja_app::autonomy::AutonomyService;
use umoja_app::compaction::CompactionService;
use umoja_app::goals::GoalService;
use umoja_app::harness::HarnessService;
use umoja_app::review::ReviewService;
use umoja_app::heartbeats::HeartbeatService;
use umoja_app::kernel::KernelService;
use umoja_app::messaging::MessagingService;
use umoja_app::schedules::ScheduleService;
use umoja_app::sessions::{SessionService, StartSession};
use umoja_app::skills::SkillService;
use umoja_app::subagents::SubagentService;
use umoja_app::supervisor::SupervisorService;
use umoja_app::Env;
use umoja_domain::message::MessageLimits;
use umoja_domain::prelude::*;
use umoja_infra::gates::ShellGateRunner;
use umoja_infra::paths::Paths;
use umoja_infra::runners::{self, CachingRunnerRegistry};
use umoja_infra::skills_fs::FsSkillCatalog;
use umoja_infra::stores::*;
use umoja_infra::summariser::AgentSummariser;
use umoja_infra::sys::{SystemClock, TimeOrderedIds, UnixProcessSupervisor};
...
impl App {
    pub fn build(cli: &Cli) -> Result<Self> {
        ...
        let runner_name = cli
            .runner
            .clone()
            .or_else(|| std::env::var("UMOJA_RUNNER").ok())
        ...
        let depth = std::env::var("UMOJA_MAX_DEPTH")
            .or_else(|_| std::env::var("PA_MAX_DEPTH"))
            .ok()
        ...
        Ok(Self {
            ...
            workdir,
            runner_name,
            session_override: cli
                .session
                .clone()
                .or_else(|| std::env::var("UMOJA_SESSION").ok())
                .or_else(|| std::env::var("PA_SESSION").ok()),
        })
    }

    /// Builds a kernel for a language, defaulting to `$UMOJA_KERNEL` then Rhai.
    pub fn kernel(&self, language: Option<&str>) -> Result<KernelService> {
        let language = match language {
            Some(name) => KernelLanguage::parse(name)?,
            None => match std::env::var("UMOJA_KERNEL").or_else(|_| std::env::var("PA_KERNEL")) {
                Ok(name) => KernelLanguage::parse(&name)?,
                Err(_) => KernelLanguage::Rhai,
            },
        };
        Ok(KernelService::new(
            self.env.clone(),
            umoja_infra::kernel::build(language, self.paths.clone(), self.workdir.clone())?,
