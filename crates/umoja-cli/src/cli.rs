...
#[command(
    ...
    bin_name = "umoja",
    version,
    about = "UMOJA — Pure Rust agent orchestration, persistent embedded kernel, and continual harness",
    long_about = "A persistent pure Rust embedded kernel, a continual harness, subagents, goals, \
                  heartbeats, schedules, messaging, autonomous gates and \
                  compaction — usable from Claude Code, opencode, Antigravity or a plain shell."
)]
pub struct Cli {
    /// Where state lives. Defaults to $UMOJA_HOME, $PRIME_AGENT_HOME, then ~/.umoja.
    #[arg(long, global = true, value_name = "DIR")]
    pub home: Option<String>,

    /// Which session to act on: its name or its id. Defaults to $UMOJA_SESSION, $PA_SESSION,
